...
class ODEFunc(nn.Module):
    ...
    def __init__(self):
        ...
        for m in self.NN_gamma.modules():
            if isinstance(m, nn.Linear):
...
if __name__ == '__main__':
    ...
    ### load data
    country = 'simulation'
    data_ = np.load('../data/simulation_2_3.npy')
    data_ = data_[0:500:5].reshape([1,-1,3])
    ...
    method = 'euler'##'dopri5' ##
    pred_y = odeint(func, func_m, y0, train_t, method=method).to(device)
    ...
    for kk in range(25):
        ...
        for itr in range(epoch_sub):
            # idx = np.random.choice(np.arange(train_data.shape[0]),batch_size)
            idx = np.array([0])
    ...
    torch.save(func_m.state_dict(), f'./models/func_m_{country}_{num}_{epoch_sub*kk+itr}_{device.type}.pt') ### train 1800 iters, loss=0.001
