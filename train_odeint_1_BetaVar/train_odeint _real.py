...
def save_fig(func, func_m, country, num, iteration, loss):
    T = torch.linspace(0., 25, 100).to(device)
    ...
    dist = norm.pdf(np.linspace(0,500,10000), loc=70, scale=1)
    ax[3].plot(K.detach().cpu().numpy()[::-1], label='dist pred')
    ax[3].plot(np.linspace(0,100,100), dist[::-1][::100], label='dist')
    ax[3].legend()
    ax[3].set_title('K')
...
T = torch.linspace(0., 25, 100).to(device)
target = torch.ones(100,1).to(device) * 3
...
if __name__ == '__main__':
    ...
    country = 'Singapore'##'South Korea' #'South Africa'##
    
    if country=='South Korea':
        data_ = np.load('../data/South Korea.npy')
        data_ = data_[0:300:3].reshape([1,-1,1])
    elif country=='Singapore':
        data_ = np.load('../data/Singapore.npy')*3
        data_ = data_[0:300:3].reshape([1,-1,1])
    # elif country=='Azerbaijan':
    #     data_ = np.load('../data/Azerbaijan.npy')
    elif country=='South Africa':
        data_ = np.load('../data/South Africa.npy')*50
        data_ = data_[0:500:5].reshape([1,-1,1])
    elif country=='Kenya':
        data_ = np.load('../data/Kenya.npy')*500
        data_ = data_[0:500:5].reshape([1,-1,1])
    elif country=='Malta':
        data_ = np.load('../data/Malta.npy')*5
        data_ = data_[0:300:3].reshape([1,-1,1])
    elif country=='Germany':
        data_ = np.load('../data/Germany.npy')*5
        data_ = data_[0:300:3].reshape([1,-1,1])
    ...
    for kk in range(20):
        ...
        for itr in range(epoch_sub):
            ...
            if itr%500==0:
                print(f'itr: {epoch_sub*kk+itr}, loss: {loss.item():.6f}')
                save_fig(func, func_m, country, num, iteration=epoch_sub*kk+itr, loss=loss)
                try:
