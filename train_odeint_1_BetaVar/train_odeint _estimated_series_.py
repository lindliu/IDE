...
def func_initialization(func, func_m, batch_t, batch_y, method, max_evals):
    ...
    best = hyper_min_3(c_func, c_func_m, batch_t, batch_y, method, init, max_evals=max_evals)
...
if __name__ == '__main__':
    ...
    for num in range(156,250,dis):
        ...
        ### set logger
        file = os.path.join(f'./models/{file_name}_loss.txt')
        ...
        best = hyper_min_2(c_func, func_m, batch_t, batch_y, method=method, max_evals=100)
        beta_init = best['beta']
        ###############################################
        ...
        for kk in range(10):
            ...
            func, func_m = func_initialization(func, func_m, batch_t, batch_y, method, max_evals=100)
            ...
            if diff[:,-cop_idx:].sum()/diff.sum()>.1:
                print("retrain beta!!!")
                pred = func.NN_beta(T.reshape([-1,1]))
                target = torch.ones(length,1).to(device) * beta_init
