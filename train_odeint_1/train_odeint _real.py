...
if __name__ == '__main__':
    ...
    if Erlang==True:
        ...
        data_ = np.load('../data/Singapore.npy')
        data_ = data_[0:300:3].reshape([1,-1,1])
        
        # data_ = np.load('../data/nn.npy')
        # data_ = data_[0:500:5].reshape([1,-1,1])*20
    ...
    ax[0].plot(pred_y[0,:,[0,2]].detach().cpu(), label=['S predict', 'R predict'])
    # ax[0].plot(data_[0,:,[0,2]])
    # ax[0].plot(batch_y[0,:,[0,2]].detach().cpu())
    ax[0].legend()
    ax[0].set_title('S and R')
    
    ax[1].plot(pred_y[0,:,1].detach().cpu(), label='I predict')
    ax[1].plot(data_[0,:,1], label='I data')
    ax[1].plot(batch_y[0,:,1].detach().cpu(), label='I train')
    ax[1].legend()
    ax[1].set_title('I')

    # ax[2].plot(pred_y[0,:,2].detach().cpu())
    # ax[2].plot(data_[0,:,2])
