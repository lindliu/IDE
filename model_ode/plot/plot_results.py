...
xlable = ['a', 'b', 'c', 'd', 'e', 'f']
fontsize = 12
fig, ax = plt.subplots(4,6, figsize=[18,15], gridspec_kw={'height_ratios': [1.5, 1., 1.5, 1]})
ax = ax.flatten()
for idx in range(6):
    ...
    if estimate:
        ax[idx].plot(time_day, data_train/N, c='r', label='Estimated j')
    else:
        ax[idx].plot(time_day, data_train/N, c='r', label='Daily average j')
    ax[idx].scatter(time_day.iloc[pred_idx], prediction_I/N, s=10, c='tab:blue', label=f'{pred_1} days prediction')
    ax[idx].scatter(time_day.iloc[pred_idx], prediction_I_/N, s=10, facecolors='none', edgecolors='tab:green', label=f'{pred_2} days prediction')
    ax[idx].legend(fontsize=fontsize)
    plt.setp(ax[idx].get_xticklabels(), rotation=45)
    ax[idx].set_ylim(0, ax[idx].axes.get_ylim()[1]*1.3)
    ax[idx].set_xlabel(f"({xlable[idx]}1)",fontsize=fontsize)
    
    
    ax[idx+6].plot(time_day.iloc[pred_idx], mu_list, linestyle='dashed', marker='o', markersize=4, label='$\mu$')
    n = 3 ## how many sigmas
    ...
    ax[idx+6].legend(fontsize=fontsize)
    if estimate:
    ...
    ax[idx+6].set_ylim(0,600)
    
    # ax[idx+6*2].scatter(time_day.iloc[np.arange(pos, idx_end-start+53)], \
    #                 pred[0,pos:pos+53,1]/N, c='tab:blue',alpha=.1,s=20)
    if estimate:
        ax[idx+6*2].plot(time_day, data_train/N, c='r', label='Estimated j')
    else:
        ax[idx+6*2].plot(time_day, data_train/N, c='r', label='Daily average j')
    ax[idx+6*2].legend(fontsize=fontsize)
    ...
    ax[idx+6*3].scatter(time_day.iloc[peak_st_idx[-1]], time_day.iloc[model_st_date[-1]], c='b', s=8)
    ax[idx+6*3].scatter(time_day.iloc[peak_nd_idx[-1]], time_day.iloc[model_nd_date[-1]], c='b', s=8)
    ...
    ax[idx+6*3].axhspan(ymin=time_day[np.clip(peak_st[idx]+start-6-pred_length,start,start+400)], ymax=time_day[peak_st[idx]-6+start], alpha=0.3, color='gray')
    ax[idx+6*3].axhspan(ymin=time_day[np.clip(peak_nd[idx]+start-6-pred_length,start,start+400)], ymax=time_day[peak_nd[idx]-6+start], alpha=0.3, color='gray')

    plt.setp(ax[idx+6*3].get_xticklabels(), rotation=45)
    plt.setp(ax[idx+6*3].get_yticklabels(), rotation=45)
...
fig.tight_layout()
fig.savefig(f'./figures/prediction_actual.png', \
