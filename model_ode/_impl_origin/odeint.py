...
def odeint(func, func_m, y0, t, *, rtol=1e-7, atol=1e-9, method=None, options=None, event_fn=None):
    ...
    if func_m.sigma.item()<0.03:
        ...
        dt_ = 0.01
    # dt_ = min(0.01, abs(func_m.sigma.item())/3)
    ...
    ##nomalization
    dt_num_f = int(400*1.2/14/dt_)+1
    t_f = torch.linspace(0,400*1.2/14,dt_num_f).to(device) * func.tau
    normalize_term = func_m(t_f.reshape(-1,1))[1:].sum()*t_f[1].item()
    K = K/(normalize_term+1e-8)
    ...
    assert method in ['euler', 'midpoint'],  "only euler and midpoint have been implemented" ### 'dopri5'
    solver = SOLVERS[method](func=func, y0=y0, K=K, rtol=rtol, atol=atol, **options)

    if event_fn is None:
        solution = solver.integrate(t, t_)
    else:
