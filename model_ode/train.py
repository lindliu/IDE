...
def save_fig(func, func_m, file_name, iteration, loss, batch_y, N, t_end, data_, length=300):
    ...
    os.makedirs(f'./figures_save/{file_name}',exist_ok=True)
    np.savez(f'./figures_save/{file_name}/{iteration}.npz', train=batch_y.cpu().numpy(), pred=pred_y, K=K, \
             sigma=func_m.sigma.item()*sc, mu=func_m.mu.item()*sc, beta=beta, tau=tau)
    fig.savefig(f'./figures_save/{file_name}/{iteration}.png', bbox_inches='tight', pad_inches=0)
    plt.close()
