...
for idx in range(6):
    ...
    if estimate:
        ...
        ax[idx].plot(time_day, data_train/N, c='r', label='Daily average I')
    ax[idx].scatter(time_day.iloc[pred_idx], prediction_I/N, s=20, c='tab:blue', label=f'{pred_1} days predict I')
    ax[idx].scatter(time_day.iloc[pred_idx], prediction_I_/N, s=20, facecolors='none', edgecolors='tab:green', label=f'{pred_2} days predict I')
    ax[idx].legend()
    plt.setp(ax[idx].get_xticklabels(), rotation=45)
