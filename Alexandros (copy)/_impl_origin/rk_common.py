...
def integration(solution, K, t__, t, t_, j):
    ...
    dt_new = .1 #(t_[j]/100).item() #
    # print(t_[j].item(), dt_new)
    points_to_interp = [torch.arange(0, t__[j].item(), dt_new).to(device)]
    # print('points: ', points_to_interp)
    K_inv = torch.flip(K, dims=(0,))#.clone()
    K_inter = RegularGridInterpolator([t_], K_inv.flatten()) ####should be inversed ????????
    K_new = K_inter(points_to_interp)
    K_new = torch.flip(K_new, dims=(0,))
    # print('fffasdf', K_new)
    # print(t[:j].shape, I.shape)
    I_inter = RegularGridInterpolator([t__[:j]], I.flatten())
...
def _runge_kutta_step(func, y0, f0, t0, dt, t1, solution, K, t, t_, j, tableau):
    ...
    for i, (alpha_i, beta_i) in enumerate(zip(tableau.alpha, tableau.beta)):
        ...
        t__[j] = ti
        # print(j,t_[j],t_)
        integro = integration(solution, K, t__, t, t_, j)
...
class RKAdaptiveStepsizeODESolver(AdaptiveStepsizeEventODESolver):
    ...
    def _advance(self, next_t, solution, K, t, t_, j):
        """Interpolate through the next time point, integrating as necessary."""
        n_steps = 0
        while next_t > self.rk_state.t1:
            assert n_steps < self.max_num_steps, 'max_num_steps exceeded ({}>={})'.format(n_steps, self.max_num_steps)
            self.rk_state = self._adaptive_step(self.rk_state, solution, self.K, t, t_, j)
            n_steps += 1
    ...
    def _adaptive_step(self, rk_state, solution, K, t, t_, j):
        ...
        y1, f1, y1_error, k = _runge_kutta_step(self.func, y0, f0, t0, dt, t1, solution, K, t, t_, j, tableau=self.tableau)
        # dtypes:
